...
# This is a single crate (not a workspace).
#
# Updates (only these sites):
#   1. [package] version in Cargo.toml
#   2. Cargo.lock entry for name = "symcourse"
...
report_versions() {
  ...
  if [[ -f Cargo.lock ]]; then
    ...
    if [[ "$lock" == "$expected" ]]; then
      printf "  %-28s  %-12s  %-12s\n" "Cargo.lock (symcourse)" "$lock" "ok"
    else
      printf "  %-28s  %-12s  %-12s\n" "Cargo.lock (symcourse)" "${lock:--}" "want ${expected}"
      ok=0
    fi
  else
    printf "  %-28s  %-12s  %-12s\n" "Cargo.lock (symcourse)" "-" "missing"
    ok=0
  fi
...
bump_cargo_lock() {
  ...
  tmp="$(mktemp)"
  awk -v new="$new" '
    $0 ~ /^name = "symcourse"$/ { hit=1; print; next }
    hit && $0 ~ /^version = "/ {
      if (match($0, /"[^"]+"/)) {
        print substr($0, 1, RSTART) new substr($0, RSTART + RLENGTH - 1)
        hit=0
...
EOF
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "--- dry-run: would insert CHANGELOG stub for [${new}] ---"
    echo "$stub"
    return 0
  fi
  local tmp
  ...
  awk -v stub="$stub" '
    ...
    in_unreleased && (/^## \[/ || /^## Version Links/) && !inserted {
      print stub
      inserted = 1
      in_unreleased = 0
    }
  ...
  ' CHANGELOG.md >"$tmp"
  awk -v ver="$new" -v url="${REPO_TAG_URL}" '
    BEGIN { done = 0 }
    /^## Version Links/ && !done {
      print
      print ""
